WITH xsdd_details AS (
	...
	U8dd AS (SELECT d.id,d.销售订单号,d.项目编码,d.项目名称, COALESCE(d.产品类别,9) 产品类别,sum(d.价税合计) 订单金额
		FROM xsdd_details d
		GROUP BY d.id,d.销售订单号,d.项目编码,d.项目名称,d.产品类别
		),
	d AS (SELECT id, mainid, yqts, ywbl, bz, sl, cplb, fkbl, fkje, fktj
	...
	OAHT AS (SELECT t.htbh,  t.htmc, t.xmbm, t.xmmc,t.htje,t.khmc,d.cplb, d.fktj,m.name ,d.bz,d.ywbl,d.yqts,d.fkbl
