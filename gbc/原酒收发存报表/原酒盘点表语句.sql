 ...
 COALESCE(b.QUANTITY_IN,0) QUANTITY_IN,COALESCE(b.AQUANTITY_IN,0) AQUANTITY_IN,COALESCE(b.QUANTITY_OUT,0) QUANTITY_OUT,COALESCE(b.AQUANTITY_OUT,0) AQUANTITY_OUT
  ...
  where B.DR = 0 
   ...
   GROUP BY t.jarscode,t.WINECODE   
	...
	z as (
	select z1.jarscode ,z1.WINECODE,z1.cname ,z1.Inventory_category,z1.Receiving,z1.dispatch,z1.Stock
	from ( select m.jarscode ,m.WINECODE,p.cname ,p.Inventory_category,
	   ...
	   where  (z1.Receiving > 0 and z1.Stock <> 0 or z1.dispatch > 0 and z1.Stock <> 0 or z1.Receiving = 0 and z1.dispatch = 0 and z1.Stock = 0 ) AND z1.jarscode <> '0'
	)
select  j.ccode ,z.WINECODE,z.cname ,z.Inventory_category,z.Receiving,z.dispatch,z.Stock
from fr_rw_wine_jars j -- 用酒坛酒罐档案表关联，能够把没有酒的空坛空罐显示出来
left join z on j.ccode = z.jarscode 
