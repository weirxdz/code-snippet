...
select  j.cwhcode ,w.CWHNAME 酒库,j.ccode 酒坛号,z.WINECODE 原酒编码,z.cname 原酒名称,z.Inventory_category inventory_category,
	p.order,
	h.TEAMNUMBER 酒班,
	h.FIRSTDATE_IN 首次入库日期,
	h.BALANCE_STRENGTH 结存度数,
	h.SEALED_STRENGTH 封坛度数,
	z.jc_AQUANTITY 实际重量,
	z.jc_QUANTITY 65度重量,
	case
		when h.INSPECTION_FLAG = 0 then '未报检'
		when h.INSPECTION_FLAG = 1 then '在检'
		else '其他'
	end 检验标识
from fr_rw_wine_jars j -- 用酒坛酒罐档案表关联，能够把没有酒的空坛空罐显示出来
left join z on j.ccode = z.jarscode and z.Stock <> 0
left join fr_rw_warehouse w on j.CWHCODE = w.CWHCODE 
LEFT JOIN fr_rw_rds_product p on z.WINECODE=p.ccode
left join fr_rw_jars_balance_h h on j.CCODE = h.JARSCODE and h.ILATEST = 1 and z.WINECODE  = h.WINECODE
where j.cwhcode <> '8888'
	and left(j.ccode,2) in('20','10') and left(z.WINECODE,4) in ('4111','1321') and z.WINECODE<>'41119999'
	${if(len(酒库) == 0,"","and j.cwhcode in ('" + 酒库 + "')")}
	${if(len(容器号) == 0,"","and j.JARSCODE in ('" + 容器号 + "')")}
	${if(len(原酒名称) == 0,"","and z.CNAME in ('" + 原酒名称 + "')")}
