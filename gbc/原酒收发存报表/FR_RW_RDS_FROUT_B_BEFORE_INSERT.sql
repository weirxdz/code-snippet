CREATE DEFINER=`blog`@`%` TRIGGER `FR_RW_RDS_FROUT_B_BEFORE_INSERT` BEFORE INSERT ON `fr_rw_rds_frout_b` FOR EACH ROW BEGIN
	...
	/*查询当前坛罐是否有存在最新的入库记录*/
	SET IFIRSTDATE_IN := (select  max(H.FIRSTDATE_IN) FROM FR_RW_JARS_BALANCE_H H WHERE H.JARSCODE = NEW.JARSCODE AND H.ILATEST = 1);
	/*查询当前坛罐最新的有效ID*/
	SET IID := (SELECT max(H.ID) FROM FR_RW_JARS_BALANCE_H H WHERE H.JARSCODE = NEW.JARSCODE AND H.ILATEST = 1);
	...
	*/
	...
	IF IFIRSTDATE_IN IS NOT NULL THEN
		-- 结存酒精度数
		SET IBALANCE_STRENGTH := (SELECT   COALESCE(H.BALANCE_STRENGTH,0)  FROM FR_RW_JARS_BALANCE_H H WHERE H.ID = IID);		
		-- 结存实际重量
		...
		-- 结存批号
		SET IBATCH := (SELECT H.CBATCH FROM FR_RW_JARS_BALANCE_H H WHERE H.ID = IID);
		IF NEW.ACTUAL_WEIGHT > 0 THEN -- 蓝字出库
      IF IBALANCE_STRENGTH = 0 THEN -- 酒精度数为空，应该是勾调虚拟罐的出入 20240830
        IF IBALANCE_QUANTITY = NEW.CONVERT_65_WEIGHT AND IBALANCE_AQUANTITY = NEW.ACTUAL_WEIGHT THEN -- 出库65度重量与实际重量都跟结存一致
          UPDATE FR_RW_JARS_BALANCE_H	SET AQUANTITY_BALANCE = IBALANCE_AQUANTITY - NEW.ACTUAL_WEIGHT, QUANTITY_BALANCE = IBALANCE_QUANTITY - NEW.CONVERT_65_WEIGHT, CBATCH= NEW.CBATCH,ILATEST = 0,ISEMPTY = 1	WHERE ID = IID; 
        END IF ;
      END IF ;
			...
			ELSEIF IBALANCE_STRENGTH = NEW.ACTUAL_STRENGTH THEN -- 出库酒精度数与结存酒精度数一致，不需要计算混合酒度和重量
				...
				ELSEIF IBALANCE_AQUANTITY = NEW.ACTUAL_WEIGHT THEN
					IF IBALANCE_QUANTITY > NEW.CONVERT_65_WEIGHT THEN 
						SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '出库65度重量超过结存65度重量,请修改出库65度重量!';
