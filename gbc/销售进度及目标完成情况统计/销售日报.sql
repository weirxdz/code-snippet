BEGIN
	...
	BEGIN/*变量赋值计算*/
		set @sdate = '${查询日期}'
		set @sdate = CONVERT(varchar(100),dateadd(dd,-1,@SDATE), 23)
	...
	BEGIN/*查询*/
		with #销售开单 as (
			SELECT x.dqbm as 大区编码,x.xshdq as 大区,x.bmbm AS 维度部门编码,x.xshbm AS 维度部门名称,cd.CCUSCODE AS 维度客户编码,cd.CCUSNAME AS 维度客户名称,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) = @sdate THEN B.INATSUM ELSE 0 END) AS 当日开单,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) = @sdate THEN B.IQUANTITY ELSE 0 END) AS 当日销量,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m AND CONVERT(varchar(100), H.DDATE, 23) <= @edate THEN B.INATSUM ELSE 0 END)  AS 当月开单,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m AND CONVERT(varchar(100), H.DDATE, 23) <= @edate THEN B.IQUANTITY ELSE 0 END)  AS 当月销量,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m_0 AND CONVERT(varchar(100), H.DDATE, 23) <= @edate_0 THEN B.INATSUM ELSE 0 END)  AS 同期开单,
				SUM(CASE WHEN CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m_0 AND CONVERT(varchar(100), H.DDATE, 23) <= @edate_0 THEN B.IQUANTITY ELSE 0 END)  AS 同期销量
			...
			WHERE left(B.CINVCODE,1)='5'
				...
				and (CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m_0 AND CONVERT(varchar(100), H.DDATE, 23) <= @edate_0
					or CONVERT(varchar(100), H.DDATE, 23) >= @sdate_m AND CONVERT(varchar(100), H.DDATE, 23) <= @edate)
				AND h.cSBVCode <> '202303270002'-- 20230331 单独排除调账的一张现结发票
			GROUP BY x.dqbm,x.xshdq,x.bmbm,x.xshbm,cd.CCUSCODE,cd.CCUSNAME
		),
		#现金收款_details as (
			SELECT x.dqbm as 大区编码,x.xshdq as 大区,x.bmbm AS 维度部门编码,x.xshbm AS 维度部门名称,cd.CCUSCODE AS 维度客户编码,cd.CCUSNAME AS 维度客户名称,
				SUM(CASE WHEN CONVERT(varchar(100), H.DVOUCHDATE, 23) = @sdate THEN	(CASE WHEN sbvb.iNatSum is NULL THEN B.IAMT ELSE ad.icamount END)	ELSE 0 END) AS 当日现金收款,
    ...
    #现金收款 as (
			SELECT 大区编码,大区,维度部门编码,维度部门名称,维度客户编码,维度客户名称,
				sum(当日现金收款) as 当日现金收款,
			  sum(当月现金收款) as 当月现金收款,
			  sum(同期现金收款) as 同期现金收款
			from #现金收款_details
			group by 大区编码,大区,维度部门编码,维度部门名称,维度客户编码,维度客户名称
		),
		#销售回款_details as (
			SELECT x.dqbm as 大区编码,x.xshdq as 大区,x.bmbm AS 维度部门编码,x.xshbm AS 维度部门名称,cd.CCUSCODE AS 维度客户编码,cd.CCUSNAME AS 维度客户名称,
				SUM(CASE WHEN CONVERT(varchar(100), H.DVOUCHDATE, 23) = @sdate THEN CASE WHEN sbvb.iNatSum is NULL THEN B.IAMT ELSE ad.icamount END ELSE 0 END) AS 当日销售回款,
			...
			WHERE ISNULL(H.CDEFINE9,'') <> '押金' /*只取非押金类的记录*/
				...
				--AND left(ISNULL(sbvb.cInvCode, '5'),1) = '5'/*如果关联发票，只取存货编码以5开头的记录*/
				AND B.CCUSVEN not in( '010124004'/*市场部（古）*/, '040000717'/*四川圆明园*/)
				and (CONVERT(varchar(100), H.DVOUCHDATE, 23) >= @sdate_m_0 AND CONVERT(varchar(100), H.DVOUCHDATE, 23) <= @edate_0
					or CONVERT(varchar(100), H.DVOUCHDATE, 23) >= @sdate_m AND CONVERT(varchar(100), H.DVOUCHDATE, 23) <= @edate)
				AND (h.vt_id = '8055' and h.cvouchtype = 49 or h.vt_id = '8052' and h.cvouchtype = 48)
				AND H.cVouchID <> '0000000744'
        AND isnull(H.cDefine2,'N') <> 'Y' -- 20230726 增加排除统计的收款单标识
			GROUP BY x.dqbm,x.xshdq,x.bmbm,x.xshbm,cd.CCUSCODE,cd.CCUSNAME
      ...
      -- 20230427 增加指定为现金收款的凭证记录
      SELECT x.dqbm as 大区编码,x.xshdq as 大区,x.bmbm AS 维度部门编码,x.xshbm AS 维度部门名称,cd.CCUSCODE AS 维度客户编码,cd.CCUSNAME AS 维度客户名称,
        SUM(CASE WHEN CONVERT(varchar(100), g.dbill_date, 23) = @edate THEN g.mc - g.md ELSE 0 END) AS 当日销售回款,
      ...
      -- 手工录入的收款信息 20230525
      select x.dqbm as 大区编码,x.xshdq as 大区,x.bmbm AS 维度部门编码,x.xshbm AS 维度部门名称,cd.CCUSCODE AS 维度客户编码,cd.CCUSNAME AS 维度客户名称,
        SUM(CASE WHEN CONVERT(varchar(100), t.ddate, 23) = @edate  THEN t.nmny ELSE 0 END) AS 当日销售回款,
      ...
      GROUP BY x.dqbm,x.xshdq,x.bmbm,x.xshbm,cd.CCUSCODE,cd.CCUSNAME
		...
		#销售回款 as (
			SELECT 大区编码,大区,维度部门编码,维度部门名称,维度客户编码,维度客户名称,
				sum(当日销售回款) as 当日销售回款,
			  sum(当月销售回款) as 当月销售回款,
			  sum(同期销售回款) as 同期销售回款
			from #销售回款_details
			group by 大区编码,大区,维度部门编码,维度部门名称,维度客户编码,维度客户名称
		),
		#dim as (
		...
		SELECT #dim.大区编码,#dim.大区,#dim.维度部门编码,#dim.维度部门名称,#dim.维度客户编码,#dim.维度客户名称,
			#销售开单.当日开单,#销售开单.当日销量,#销售开单.当月开单,#销售开单.当月销量,#销售开单.同期开单,#销售开单.同期销量,
			#销售回款.当日销售回款,#销售回款.当月销售回款,#销售回款.同期销售回款,
			...
			WHERE isnull(#销售回款.当月销售回款,0) <> 0 or isnull(#销售回款.同期销售回款,0) <> 0
				or isnull(#现金收款.当月现金收款,0) <> 0 or isnull(#现金收款.同期现金收款,0) <> 0
				or isnull(#销售开单.当月开单,0) <> 0 or isnull(#销售开单.同期开单,0) <> 0
				or isnull(#销售开单.当月销量,0) <> 0 or isnull(#销售开单.同期销量,0) <> 0
	END
