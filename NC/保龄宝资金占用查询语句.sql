...
t5 as (
  select distinct cust.code 客户编码, cust.name 客户名称,m.code 物料编码, m.name 物料名称, sum(t.local_money_de) over(partition by cust.code,m.code ) 按客户物料汇总处理金额
             from arap_verifydetail t
                ...
                where substr(t.busidate,1,10)>='2023-01-01' and substr(t.busidate,1,10)<=substr(to_char(sysdate,'yyyy-mm-dd'),1,10) and szxm.name ='运费（产品、回桶、出口）'
...
fr_details as (
  select t.核销明细表主键,
       ...
       CASE WHEN t.借方数量 <> 0 THEN round(t1.直接运费 * t.借方处理数量 / t.借方数量, 6) ELSE 0 end 直接运费分摊金额,
       t2.客户汇总回桶运费,
       ...
       t2.客户回桶运费总额,
       t2_1.无核销客户的回桶运费,
       CASE WHEN (sum(case when t.是否售桶 = '周转桶' then t.借方处理数量 else 0 end) over()) = 0 THEN 0 ELSE  round(t2_1.无核销客户的回桶运费 * (case when t.是否售桶 = '周转桶' then t.借方处理数量 else 0 end) / (sum(case when t.是否售桶 = '周转桶' then t.借方处理数量 else 0 end) over()),6) END 无核销客户分摊回桶运费,
       sum(t.借方处理数量) over(partition by t.销售部门) 按部门汇总处理数量,
       t3.部门调拨运费,CASE WHEN (sum(t.借方处理数量) over(partition by t.销售部门)) = 0 THEN 0 ELSE round(t3.部门调拨运费*t.借方处理数量/(sum(t.借方处理数量) over(partition by t.销售部门)) ,6) END 分摊调拨运费,
       t4.按客户汇总退货运费,
       sum(t.借方处理数量) over(partition by t.客户编码) 按客户汇总处理数量,
       CASE WHEN sum(t.借方处理数量)over(partition by t.客户编码) = 0 THEN 0 ELSE round(t4.按客户汇总退货运费 * t.借方处理数量 / sum(t.借方处理数量)over(partition by t.客户编码),6) END 客户分摊退货运费,
       t4_1.部门汇总退货运费,
       CASE WHEN (sum(t.借方处理数量) over(partition by t.销售部门)) = 0 THEN 0 ELSE round(t4_1.部门汇总退货运费 * t.借方处理数量 /(sum(t.借方处理数量) over(partition by t.销售部门)),6) END 按部门分摊退货运费,
...
select t.核销明细表主键,
       ...
       CASE WHEN t.处理金额按客户物料汇总 = 0 THEN 0 ELSE round(t.借方处理本币金额* t.代垫海运费处理金额/t.处理金额按客户物料汇总,2) END 分摊后的代垫运费
from fr_details t
