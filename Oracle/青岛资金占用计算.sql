with fr_v_verifydetail_ys as (
  select psn.name 业务员,t.pk_verifydetail 核销明细表主键,
    ...
    where /*substr(t.busidate,1,10)>='' and substr(t.busidate,1,10)<='' and */t.billclass = 'ys' and t.dr = 0 and t.busiflag <> 2 and h.src_syscode <> 0
          ...
          and substr(saleout_b.dbizdate,1,10) >= '2023-01-01'
  ...
  select t.zj 主键, t.dbizdate 出库日期, sum(t.ftyfje) 直接运费
    from (select b.cgeneralbid zj,
          ...
          where b.dr = 0
            ...
            and dm_b.dr = 0
            and b.nnum > 0
            and substr(b.dbizdate, 1, 10) >= '2023-01-01'
...
t2 as (-- 当期有核销客户的回桶运费
  SELECT distinct
    ...
    WHERE h.dr = 0  AND b.dr = 0  AND m.name = '塑料桶'
        ...
        and b.pk_org = '0001B31000000039DYB9'
),
t31 as (
  SELECT
  ...
  select distinct cust.code 客户编码, cust.name 客户名称,m.code 物料编码, m.name 物料名称, sum(t.local_money_de) over(partition by cust.code,m.code ) 按客户物料汇总处理金额
             from arap_verifydetail t
                ...
                where substr(t.busidate,1,10)>='2023-01-01' and substr(t.busidate,1,10)<=substr(to_char(sysdate,'yyyy-mm-dd'),1,10) and szxm.name ='运费（产品、回桶、出口）'
                      and recitem.pk_org = '0001B31000000039DYB9'
),
t_st as (
...
select t.核销明细表主键,
      ...
      --  t.代垫海运费处理金额,
      --  t.处理金额按客户物料汇总,
      --  round(t.直接运费分摊金额+t.客户分摊回桶运费+t.分摊调拨运费+t.客户分摊退货运费+t.按部门分摊退货运费,2) 运费汇总,
